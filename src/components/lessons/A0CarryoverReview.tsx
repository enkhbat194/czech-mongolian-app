...
const A0CarryoverReview: React.FC<A0CarryoverReviewProps> = ({ lessonId, onComplete }) => {
  ...
  const choose = (id: string) => {
    ...
    timerRef.current = window.setTimeout(() => {
      if (index >= targets.length - 1) onComplete();
      else {
  ...
  return (
    <div style={{ background: '#0C0C0E', minHeight: '100dvh', color: '#FFF', fontFamily: 'Inter,sans-serif', padding: 'max(20px, env(safe-area-inset-top)) 18px 30px' }}>
      <div style={{ maxWidth: 430, margin: '0 auto', minHeight: '78dvh', display: 'flex', alignItems: 'center' }}>
        <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} style={{ width: '100%', background: '#1C1C1F', border: '1px solid #2A2A2F', borderRadius: 24, padding: 21 }}>
          <div style={{ width: 46, height: 46, borderRadius: 23, display: 'grid', placeItems: 'center', background: 'rgba(200,149,42,.15)', border: '1px solid rgba(200,149,42,.42)', marginBottom: 12 }}>
            <Volume2 size={23} color="#F5C842" />
          </div>
          ...
          <div style={{ borderRadius: 16, background: '#141416', border: '1px solid #303036', padding: '18px 15px', marginBottom: 14 }}>
            <p style={{ margin: 0, textAlign: 'center', color: '#FFF', fontSize: 21, fontWeight: 800 }}>{target.mongolian}</p>
          </div>
            ...
            {choices.map((item) => {
              ...
              return (
                <button
                  ...
                  style={{ textAlign: 'left', padding: '13px 14px', borderRadius: 14, color: '#FFF', background: right ? 'rgba(34,197,94,.16)' : wrong ? 'rgba(239,68,68,.16)' : '#242428', border: right ? '1px solid rgba(34,197,94,.65)' : wrong ? '1px solid rgba(239,68,68,.65)' : '1px solid #34343A', cursor: feedback === 'correct' ? 'default' : 'pointer', fontSize: 16 }}
                >
                  {item.czech}
                </button>
              );
          ...
          {feedback === 'correct' && <div style={{ display: 'flex', gap: 8, alignItems: 'center', marginTop: 14, color: '#4ADE80', fontWeight: 800, fontSize: 13 }}><Check size={17} /> Зөв. Энэ хэллэг дараагийн ярианд дахин гарна.</div>}
