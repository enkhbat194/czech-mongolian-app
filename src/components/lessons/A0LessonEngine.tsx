...
const DialogueRun: React.FC<{
  ...
  }, [feedback, history.length, onProgress, stepIndex]);
    ...
    <div key={id} style={{ display: 'flex', flexDirection: side === 'staff' ? 'row' : 'row-reverse', alignItems: 'flex-end', gap: 6 }}>
      ...
      <div style={{ maxWidth: '80%', background: side === 'staff' ? '#242428' : 'rgba(200,149,42,.16)', border: side === 'staff' ? '1px solid #34343A' : '1px solid rgba(200,149,42,.40)', borderRadius: 14, padding: '8px 10px' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 3 }}>
          <p style={{ margin: 0, flex: 1, color: side === 'staff' ? '#A0A0A8' : '#F5C842', fontSize: 9, fontWeight: 800 }}>{speaker}</p>
          <TinyPlay text={czech} />
        </div>
...
const A0LessonEngine: React.FC<{ config: A0LessonEngineConfig }> = ({ config }) => {
  ...
  const [selectedCzechId, setSelectedCzechId] = useState<string | null>(null);
  const [selectedMongolianId, setSelectedMongolianId] = useState<string | null>(null);
  ...
  const matchCzechItems = useMemo(() => currentExercise?.type === 'match' ? stableShuffle(currentExercise.pairs, `${currentExercise.id}-cs`) : [], [currentExercise]);
  const matchMongolianItems = useMemo(() => currentExercise?.type === 'match' ? stableShuffle(currentExercise.pairs, `${currentExercise.id}-mn`) : [], [currentExercise]);
  ...
  const resetExercise = () => {
    ...
    setOrderedTokens([]);
    setTypedText('');
    setSelectedCzechId(null);
    setSelectedMongolianId(null);
    setMatchedPairIds([]);
    setWrongMatchIds([]);
    if (matchTimerRef.current) window.clearTimeout(matchTimerRef.current);
  ...
  const submitTyping = () => {
    ...
    setFeedback(normalizeCzech(typedText) === normalizeCzech(currentExercise.targetText) ? 'correct' : 'wrong');
  };

  const attemptMatch = (czechId: string, mongolianId: string) => {
    if (!currentExercise || currentExercise.type !== 'match') return;
    if (czechId === mongolianId) {
      const next = [...matchedPairIds, czechId];
      setMatchedPairIds(next);
      setSelectedCzechId(null);
      setSelectedMongolianId(null);
      setWrongMatchIds([]);
      if (next.length === currentExercise.pairs.length) setFeedback('correct');
      return;
    }

    setWrongMatchIds([czechId, mongolianId]);
    if (matchTimerRef.current) window.clearTimeout(matchTimerRef.current);
  ...
  const selectCzechMatch = (id: string) => {
    if (!currentExercise || currentExercise.type !== 'match' || matchedPairIds.includes(id)) return;
    const pair = currentExercise.pairs.find((item) => item.id === id);
    if (pair) speakCzech(pair.czech);
    setWrongMatchIds([]);
    if (selectedMongolianId) attemptMatch(id, selectedMongolianId);
    else setSelectedCzechId(id);
  };

  const selectMongolianMatch = (id: string) => {
    if (!currentExercise || currentExercise.type !== 'match' || matchedPairIds.includes(id)) return;
    setWrongMatchIds([]);
    if (selectedCzechId) attemptMatch(selectedCzechId, id);
    else setSelectedMongolianId(id);
  };
