...
const DialogueRun: React.FC<{ scenario: DialogueScenario; onComplete: () => void; onProgress: (value: number) => void }> = ({ scenario, onComplete, onProgress }) => {
  ...
  const choose = (id: string) => {
    ...
    const isCorrect = id === step.correctId;
    setPicked(id);
...
const A0LessonEngineV3: React.FC<{ config: A0LessonEngineConfig }> = ({ config }) => {
  ...
  const choose = (id: string, correctId: string) => { setChoice(id); setFeedback(id === correctId ? 'correct' : 'wrong'); };
  const addToken = (token: string) => {
    if (!exercise || exercise.type !== 'order' || feedback === 'correct') return;
    const next = [...tokens, token]; setTokens(next);
    if (next.length === exercise.tokens.length) setFeedback(next.join(' ') === exercise.expectedText ? 'correct' : 'wrong');
  };
  const submitTyping = () => { if (exercise?.type === 'typing') setFeedback(normalize(typed) === normalize(exercise.targetText) ? 'correct' : 'wrong'); };
  ...
  return <div style={shell}>
    <header style={{ padding: 'max(16px, env(safe-area-inset-top)) 20px 13px', background: '#141416', borderBottom: '1px solid #2A2A2F' }}><div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 11 }}><button onClick={() => store.setPage('path')} style={{ width: 34, height: 34, borderRadius: 10, border: 0, background: '#242428', color: '#A0A0A8', cursor: 'pointer' }}><ChevronLeft size={20} /></button><div style={{ flex: 1 }}><p style={{ margin: 0, color: '#A0A0A8', fontSize: 12 }}>{config.titleMn.split(' — ')[0]} · {config.durationMinutes} минут</p><h1 style={{ margin: '2px 0 0', fontSize: 17 }}>{config.titleMn.split(' — ')[1] || config.titleMn}</h1></div><span style={{ color: '#C8952A', fontSize: 12, fontWeight: 800 }}>{Math.min(completed + 1, total)}/{total}</span></div><ProgressBar value={completed} max={total} height={5} /></header>
...
export default A0LessonEngineV3;