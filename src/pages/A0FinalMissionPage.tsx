...
const A0FinalMissionPage: React.FC = () => {
  ...
  const answerChoice = (choiceId: string) => {
    ...
    if (chosen) speakCzech(chosen.text);
      ...
      <main style={{ maxWidth: 430, margin: '0 auto', padding: '20px 16px 30px' }}>
        <motion.div key={question.id} initial={{ opacity: 0, x: 14 }} animate={{ opacity: 1, x: 0 }} style={panel}>
          ...
          {question.type === 'typing' ? <div>
            <input value={typingValue} onChange={(event) => setTypingValue(event.target.value)} disabled={feedback !== null} placeholder="Чехээр бич" style={{ width: '100%', boxSizing: 'border-box', padding: '14px 15px', borderRadius: 14, background: '#242428', border: '1px solid #34343A', color: '#FFF', fontSize: 16, marginBottom: 12 }} />
