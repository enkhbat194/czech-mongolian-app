...
import {
  a0FirstContactCards,
  a0FirstContactMicroLessons,
  a0FirstContactMission,
  getA0FirstContactCard,
} from '../data/a0FirstContact';
...
const A0FirstContactPage: React.FC = () => {
  ...
  const completedSteps = useMemo(() => {
    ...
    if (stage === 'exercises') return finishedMicros + currentMicro.cardIds.length + exerciseIndex;
    if (stage === 'mission') return totalSteps - a0FirstContactMission.length + missionIndex;
    return totalSteps;
  ...
  const nextExercise = () => {
    ...
    if (microIndex < a0FirstContactMicroLessons.length - 1) {
  ...
  const finish = () => {
    if (completed) return;
    a0FirstContactCards.forEach((card) => {
    ...
    updateStreak();
    completeLesson('l001');
    unlockNextLesson('l001');
    setCompleted(true);
    setStage('complete');
  };

  const currentMission = a0FirstContactMission[missionIndex];
      ...
      <div style={{ ...shell, display:'flex', alignItems:'center', padding:24 }}>
        <div style={{ ...panel, width:'100%', textAlign:'center' }}>
          ...
          <p style={{ color:'#A0A0A8', lineHeight:1.55, margin:'0 0 18px' }}>
            Та албан ёсоор мэндэлж, нэрээ болон хаанаас ирснээ хэлж, ойлгохгүй үед яриаг удаашруулах хүсэлт тавьж чадна.
          </p>
          <div style={{ ...panel, padding:16, textAlign:'left', marginBottom:16 }}>
            <p style={{ margin:'0 0 7px', color:'#C8952A', fontWeight:800 }}>Одоо таны ашиглаж чадах 5 алхам</p>
            {['Dobrý den.','Jmenuji se …','Jsem z Mongolska.','Nerozumím. Mluvte prosím pomalu.','Na shledanou.'].map((text) => (
          ...
          <button onClick={() => window.location.reload()} style={{ width:'100%', marginTop:10, padding:12, border:0, background:'transparent', color:'#A0A0A8', cursor:'pointer' }}>
            <RotateCcw size={14} style={{ verticalAlign:'middle', marginRight:6 }} /> A0.1-ийг дахин хийх
          </button>
        </div>
      </div>
    );
  ...
  const feedbackBox = (text: string) => feedback && (
    <div style={{ marginTop:14, borderRadius:14, padding:13, background:feedback === 'correct' ? 'rgba(34,197,94,.12)' : 'rgba(239,68,68,.12)', border:`1px solid ${feedback === 'correct' ? 'rgba(34,197,94,.35)' : 'rgba(239,68,68,.35)'}` }}>
      <p style={{ margin:0, color:feedback === 'correct' ? '#4ADE80' : '#F87171', fontWeight:800 }}>
        {feedback === 'correct' ? 'Зөв.' : 'Буруу. Дахин бодоорой.'}
      </p>
      ...
      <main style={{ padding:'18px 16px 28px' }}>
        ...
        {stage === 'mission' && currentMission && (
          <>
            ...
            <p style={{ color:'#A0A0A8', fontSize:13, margin:'0 0 16px', lineHeight:1.45 }}>Нөхцөл: Чехийн ажлын газрын ресепшнд анх очлоо. Зөв хариултыг сонгон яриаг гүйцээ.</p>
            <div style={panel}>
              <p style={{ margin:'0 0 8px', color:'#A0A0A8', fontSize:12 }}>Алхам {missionIndex + 1}/{a0FirstContactMission.length}</p>
              ...
              <div style={{ display:'flex', flexDirection:'column', gap:10 }}>
                {currentMission.choices.map((item) => {
                  ...
                  return <button key={item.id} onClick={() => answerChoice(currentMission.correctId, item.id)} disabled={feedback === 'correct'}
            ...
            {feedback === 'correct' && <button onClick={() => {
              if (missionIndex === a0FirstContactMission.length - 1) finish();
              else { setMissionIndex((value) => value + 1); resetAnswer(); }
            }} className="btn-gold" style={{ width:'100%', marginTop:16, padding:15, fontSize:15 }}>
              {missionIndex === a0FirstContactMission.length - 1 ? 'A0.1-ийг дуусгах' : 'Дараагийн алхам'}
            </button>}
          </>
        )}
      </main>
