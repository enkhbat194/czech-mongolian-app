...
const ProgressPage: React.FC = () => {
  const { progress, words, srsCards } = useAppStore();
    ...
    <div style={{background:'#0C0C0E',minHeight:'100vh',fontFamily:'Inter,sans-serif'}}>
          ...
          {TABS.map((t,i) => (
            <button key={t} onClick={()=>setTab(i)}
              style={{
                ...
                background: tab===i ? '#0C0C0E' : 'transparent',
                color: tab===i ? '#C8952A' : '#606068',
                fontSize:13, fontWeight:700, border:'none', cursor:'pointer',
                borderTop: tab===i ? '2px solid #C8952A' : '2px solid transparent',
      ...
      <div style={{padding:16}}>
          ...
          <div style={{display:'flex',flexDirection:'column',gap:14}}>
            ...
            <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:10}}>
              {[
                ...
                <div key={s.label} style={{background:'#1C1C1F',borderRadius:18,padding:16,
                  border:'1px solid #2A2A2F'}}>
                  <span style={{fontSize:22}}>{s.icon}</span>
                  <p style={{fontSize:22,fontWeight:900,color:'#C8952A',margin:'6px 0 2px'}}>
                    {s.val} <span style={{fontSize:11,color:'#606068'}}>{s.sub}</span>
                  </p>
                ...
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={weekData}>
                    ...
                    <Bar dataKey="xp" radius={[6,6,0,0]}>
                      {weekData.map((d,i) => (
                        <Cell key={i}
                          fill={d.xp>0 ? 'url(#goldBar)' : '#2A2A2F'}
                        />
                      ))}
                    </Bar>
                    <defs>
            ...
            <div style={{background:'#1C1C1F',borderRadius:20,padding:16,border:'1px solid #2A2A2F'}}>
              ...
              <div style={{display:'flex',justifyContent:'space-around',alignItems:'center'}}>
                <CircularProgress value={mastered} max={Math.max(1,srsCards.length)}
                  size={80} stroke={8} color="#22C55E"
                  label={`${mastered}`} sub="Эзэмшсэн"/>
                <CircularProgress value={learning} max={Math.max(1,srsCards.length)}
                  size={80} stroke={8} color="#C8952A"
                  label={`${learning}`} sub="Суралцаж"/>
                <CircularProgress value={struggling} max={Math.max(1,srsCards.length)}
                  size={80} stroke={8} color="#EF4444"
                  label={`${struggling}`} sub="Хэцүү"/>
              </div>
            </div>
            ...
            <div style={{background:'#1C1C1F',borderRadius:20,padding:16,border:'1px solid #2A2A2F'}}>
              ...
              {[
                ...
                <div key={a.label} style={{display:'flex',alignItems:'center',gap:12,
                  padding:'10px 0',borderBottom:'1px solid #2A2A2F'}}>
                  <div style={{width:40,height:40,borderRadius:12,
                    background:a.done?'rgba(200,149,42,.15)':'#242428',
                    display:'flex',alignItems:'center',justifyContent:'center',
                    fontSize:20,opacity:a.done?1:.4}}>
                    {a.icon}
                  </div>
                  <div style={{flex:1}}>
                    <p style={{fontSize:13,fontWeight:700,color:a.done?'#FFF':'#606068'}}>{a.label}</p>
                    <p style={{fontSize:11,color:'#606068'}}>{a.sub}</p>
                  </div>
                  {a.done && (
                    <div style={{background:'rgba(200,149,42,.15)',borderRadius:99,padding:'3px 10px',
                      fontSize:11,fontWeight:700,color:'#C8952A',border:'1px solid rgba(200,149,42,.3)'}}>
                      ✓
                    </div>
                  )}
                </div>
        ...
        {tab===1 && (
          <div style={{display:'flex',flexDirection:'column',gap:14}}>
            <div style={{background:'#1C1C1F',borderRadius:20,padding:16,border:'1px solid #2A2A2F'}}>
              <p style={{fontSize:13,color:'#A0A0A8',marginBottom:6}}>Өнөөдөр давтах</p>
              <p style={{fontSize:28,fontWeight:900,color:'#C8952A',marginBottom:4}}>{due.length}</p>
              <ProgressBar value={mastered} max={Math.max(1,srsCards.length)} height={8} showPct/>
            </div>
            {due.length > 0 && (
              <div style={{background:'#1C1C1F',borderRadius:20,padding:16,border:'1px solid #2A2A2F'}}>
                <p style={{fontSize:13,fontWeight:700,color:'#FFF',marginBottom:10}}>Өнөөдрийн давтах ({due.length})</p>
                {due.slice(0,5).map(c => {
                  const w = words.find(x=>x.id===c.wordId);
                  return w ? (
                    <div key={c.wordId} style={{display:'flex',justifyContent:'space-between',
                      alignItems:'center',padding:'9px 0',borderBottom:'1px solid #2A2A2F'}}>
                      <div>
                        <p style={{fontSize:14,fontWeight:700,color:'#FFF'}}>{w.czech}</p>
                        <p style={{fontSize:11,color:'#606068'}}>{w.mongolian}</p>
                      </div>
            ...
            {upcoming.length > 0 && (
              <div style={{background:'#1C1C1F',borderRadius:20,padding:16,border:'1px solid #2A2A2F'}}>
                <p style={{fontSize:13,fontWeight:700,color:'#FFF',marginBottom:10}}>Маргааш давтах</p>
                {upcoming.map(c => {
                  const w = words.find(x=>x.id===c.wordId);
                  return w ? (
                    <div key={c.wordId} style={{display:'flex',justifyContent:'space-between',
            ...
            {words.slice(0,30).map((w,i) => {
              ...
              return (
                <div key={w.id} style={{display:'flex',alignItems:'center',gap:12,
                  padding:'12px 16px',borderBottom:i<29?'1px solid #2A2A2F':'none'}}>
                  <div style={{width:36,height:36,borderRadius:10,
                    background: learned?'rgba(200,149,42,.15)':'#242428',
                    display:'flex',alignItems:'center',justifyContent:'center',fontSize:11,
                    fontWeight:700,color:learned?'#C8952A':'#606068',flexShrink:0}}>
                    {i+1}
                  </div>
                  <div style={{flex:1}}>
                    <p style={{fontSize:14,fontWeight:700,color:'#FFF'}}>{w.czech}</p>
                  ...
                  {learned && (
                    <div style={{fontSize:11,fontWeight:700,color:'#C8952A',
                      background:'rgba(200,149,42,.1)',padding:'3px 8px',borderRadius:99}}>
                      Lv {card?.repetitions||0}
                    </div>
                  )}
                </div>
              );
