...
const ProgressPage: React.FC = () => {
  ...
  const due = srsCards.filter((card) => {
  ...
  const upcoming = srsCards.filter((card) => {
    if (!card.nextReview) return false;
    return getLocalDateKey(new Date(card.nextReview)) === tomorrowKey;
  });
      ...
      <div style={{ padding: 16 }}>
        {tab === 0 && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
            ...
            <div style={{ background: '#1C1C1F', borderRadius: 20, padding: 16, border: '1px solid #2A2A2F' }}>
              <p style={{ fontSize: 13, fontWeight: 700, color: '#FFF', margin: '0 0 12px' }}>7 өдрийн ахиц</p>
              <div style={{ height: 120, display: 'grid', gridTemplateColumns: 'repeat(7, 1fr)', gap: 8, alignItems: 'end' }}>
                {weekData.map((day) => {
                  const barHeight = Math.max(8, Math.round((day.xp / maxWeeklyXp) * 88));
                  return (
                    <div key={day.d} style={{ height: 120, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'flex-end', gap: 6 }}>
                      <div style={{ color: day.xp > 0 ? '#F5C842' : '#606068', fontSize: 10, fontWeight: 800, minHeight: 14 }}>{day.xp}</div>
                      <div style={{ width: '100%', height: barHeight, borderRadius: '8px 8px 3px 3px', background: day.xp > 0 ? 'linear-gradient(180deg,#F5C842,#C8952A)' : '#2A2A2F' }} />
        ...
        {tab === 1 && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
            ...
            {due.length > 0 && <div style={{ background: '#1C1C1F', borderRadius: 20, padding: 16, border: '1px solid #2A2A2F' }}><p style={{ fontSize: 13, fontWeight: 700, color: '#FFF', marginBottom: 10 }}>Өнөөдрийн давтах ({due.length})</p>{due.slice(0, 5).map((card) => { const w = words.find((item) => item.id === card.wordId); return w ? <div key={card.wordId} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '9px 0', borderBottom: '1px solid #2A2A2F' }}><div><p style={{ fontSize: 14, fontWeight: 700, color: '#FFF' }}>{w.czech}</p><p style={{ fontSize: 11, color: '#606068' }}>{w.mongolian}</p></div><div style={{ width: 8, height: 8, borderRadius: 4, background: '#EF4444' }} /></div> : null; })}</div>}
            {upcoming.length > 0 && <div style={{ background: '#1C1C1F', borderRadius: 20, padding: 16, border: '1px solid #2A2A2F' }}><p style={{ fontSize: 13, fontWeight: 700, color: '#FFF', marginBottom: 10 }}>Маргааш давтах</p>{upcoming.slice(0, 5).map((card) => { const w = words.find((item) => item.id === card.wordId); return w ? <div key={card.wordId} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '9px 0', borderBottom: '1px solid #2A2A2F' }}><div><p style={{ fontSize: 14, fontWeight: 700, color: '#FFF' }}>{w.czech}</p><p style={{ fontSize: 11, color: '#606068' }}>{w.mongolian}</p></div><div style={{ width: 8, height: 8, borderRadius: 4, background: '#22C55E' }} /></div> : null; })}</div>}
          </div>
        )}
        ...
        {tab === 2 && (
          <div style={{ background: '#1C1C1F', borderRadius: 20, border: '1px solid #2A2A2F', overflow: 'hidden' }}>
            {words.map((w, i) => {
              const card = srsCards.find((item) => item.wordId === w.id);
