...
const SettingsPage: React.FC = () => {
  const { userName, setUserName, progress, dailyGoalMinutes, setDailyGoal, resetProgress } = useAppStore();
    ...
    <div style={{background:'#0C0C0E',minHeight:'100vh',fontFamily:'Inter,sans-serif'}}>
      ...
      <div style={{padding:16,display:'flex',flexDirection:'column',gap:14}}>
        ...
        <div style={{background:'#1C1C1F',borderRadius:20,padding:16,border:'1px solid #2A2A2F'}}>
          <p style={{fontSize:11,fontWeight:700,color:'#606068',marginBottom:12,textTransform:'uppercase',letterSpacing:.8}}>
            Профайл
          </p>
          <div style={{padding:'14px 0',borderBottom:'1px solid #2A2A2F'}}>
            ...
            <div style={{display:'flex',gap:8}}>
              <input value={name} onChange={e=>setName(e.target.value)}
                style={{
                  flex:1, background:'#242428', border:'1px solid #2A2A2F',
                  borderRadius:10, padding:'8px 12px', color:'#FFF',
                  fontSize:14, outline:'none',
                }}
              />
              <button onClick={()=>setUserName(name)}
                className="btn-gold" style={{padding:'8px 14px',fontSize:13,borderRadius:10}}>
                Хадгалах
              </button>
            </div>
          </div>
              ...
              {[5,10,15,20,30].map(m => (
                <button key={m} onClick={()=>setDailyGoal(m)}
                  style={{
                    ...
                    borderColor: dailyGoalMinutes===m ? '#C8952A' : '#2A2A2F',
                    background: dailyGoalMinutes===m ? 'rgba(200,149,42,.15)' : '#242428',
                    color: dailyGoalMinutes===m ? '#C8952A' : '#606068',
                  }}>
                  {m} мин
                </button>
              ))}
          ...
          <div style={{paddingTop:14}}>
            ...
            <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',
              background:'#242428',borderRadius:10,padding:'10px 14px'}}>
        ...
        <div style={{background:'#1C1C1F',borderRadius:20,padding:16,border:'1px solid rgba(239,68,68,.3)'}}>
          <p style={{fontSize:11,fontWeight:700,color:'#F87171',marginBottom:8,textTransform:'uppercase',letterSpacing:.8}}>
            Аюултай бүс
          </p>
          {!confirmReset ? (
            <button onClick={()=>setConfirmReset(true)}
              style={{width:'100%',padding:12,borderRadius:12,fontSize:14,fontWeight:700,
                background:'rgba(239,68,68,.1)',border:'1.5px solid rgba(239,68,68,.3)',
                color:'#F87171',cursor:'pointer'}}>
              🗑️ Бүх өгөгдөл устгах
            </button>
          ) : (
            <div style={{display:'flex',gap:8}}>
              <button onClick={()=>setConfirmReset(false)}
                className="btn-outline" style={{flex:1,padding:12,fontSize:13}}>
                Болих
              </button>
        ...
        <p style={{textAlign:'center',fontSize:11,color:'#2A2A2F',paddingBottom:8}}>
          ЧехМонгол v1.0 · localStorage
        </p>
      </div>
    </div>
  );
